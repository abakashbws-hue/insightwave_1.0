...
def dynamic_chunking(df: pd.DataFrame, overhead_tokens: int, max_prompt_tokens: int = 100000, sample_size: int = 200):
    """
    ...
    if data_token_budget <= 0:
        print(f"  WARNING: Prompt overhead ({overhead_tokens} tokens) exceeds the max prompt token limit ({max_prompt_tokens}). This will likely fail. Chunk size set to a minimal 10 rows.")
        return 10

    # 5. Calculate how many rows can fit into the data budget.
    rows_per_chunk = int(data_token_budget / tokens_per_row) if tokens_per_row > 0 else len(df)
...
def create_blueprint_iteratively(
    df_input: pd.DataFrame, config_path: str, debug_dir: str, primary_key_column: str, user_feedback: str = ""
):
    """
    ...
    """
    print("\n--- Starting Blueprint Creation via Iterative Refinement ---")

    try:
        with open(config_path, "r") as f:
            config_data = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError) as e:
        print(f"FATAL: Could not load config file at '{config_path}'. Error: {e}")
        return None
...
"""
    ...
    **New Data Chunk to Incorporate:**
    ...
    user_feedback_prompt = ""
    if user_feedback:
        print(f"  Incorporating user feedback: {user_feedback}")
    ...
    print(f"  Calculated prompt overhead (template + config + feedback): {overhead_tokens} tokens.")

    chunk_size = dynamic_chunking(df=df, overhead_tokens=overhead_tokens)
    print(f"  The dynamic chunk size is determined to be: {chunk_size} rows.")
    ...
    for j, chunk_df in enumerate(chunk_iterator):
        print(f"\n--- Processing chunk {j+1} to refine blueprint... ---")
        chunk_json_str = chunk_df.to_json(orient="records")

        if j == 0:
            print("  This is the first chunk. Creating initial blueprint...")
            ...
            if not isinstance(current_blueprint, dict):
                print(f"    FATAL: Cannot refine blueprint because it is not a valid dictionary. Aborting.")
                return None

            print(f"  Refining existing blueprint with data from chunk {j+1}...")
        ...
        for attempt in range(max_json_retries):
            try:
                ...
                print(f"    WARNING: Failed to decode JSON for '{call_desc}' (Attempt {attempt + 1}/{max_json_retries}). Retrying...")
                if attempt + 1 >= max_json_retries:
                    print(f"    FATAL: Could not parse JSON response for '{call_desc}' after {max_json_retries} attempts. Aborting blueprint creation.")
                    utils.log_llm_error(debug_dir, prompt, e, call_desc)
                    return None
                time.sleep(5)
            except (exceptions.ServiceUnavailable, Exception) as e:
                print(f"    FATAL: Unrecoverable error during content generation for chunk {j+1}: {e}. Aborting blueprint creation.")
                return None
        ...
        if response_json:
            if j == 0:
                if not isinstance(response_json, dict):
                    print(
                        f"    FATAL: Initial blueprint response is not a valid JSON object (expected dict). Got: {type(response_json)}. Raw response: {response.text}. Aborting."
                    )
                    return None
                current_blueprint = response_json.get("migration_blueprint", response_json)
                if "migration_waves" in current_blueprint and "wave_definitions" not in current_blueprint:
                    print("  INFO: Normalizing blueprint key from 'migration_waves' to 'wave_definitions'.")
                    current_blueprint["wave_definitions"] = current_blueprint.pop("migration_waves")
                
                
                if not isinstance(current_blueprint, dict):
                    print(
                        f"    FATAL: current_blueprint is not a dictionary after initial creation attempt for chunk {j+1}. Type: {type(current_blueprint)}. Aborting."
                    )
                    return None
            else:
                current_blueprint = _merge_blueprint_changes(current_blueprint, response_json)

            refined_blueprint_filename = os.path.join(debug_dir, f"migration_blueprint_after_chunk_{j+1}.json")
            with open(refined_blueprint_filename, "w") as f:
                json.dump(current_blueprint, f, indent=4)
            print(f"  Blueprint after chunk {j+1} saved to: {os.path.abspath(refined_blueprint_filename)}")
        else:
            print(f"    FATAL: Failed to get a valid JSON response for chunk {j+1}. Aborting.")
            return None
...
def generate_migration_blueprint(tool_context: ToolContext, user_feedback: str = "") -> dict:
    """
    ...
    print(f"Using debug directory: {debug_dir}")

    primary_key_column = 'asset_name'
    print(f"  Using standard primary key: '{primary_key_column}'")
    ...
    print("\n--- Blueprint generation complete. Returning blueprint object. ---")
...
def save_blueprint(tool_context: ToolContext) -> str:
    ...
    try:
        ...
        print(f"  Approved blueprint saved to: {os.path.abspath(final_blueprint_path)}")
