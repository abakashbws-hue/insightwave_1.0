...
def update_pattern_doc(tool_context: ToolContext, destination_environment: str):
    ...
    if destination_environment not in ['GKE', 'GCVE', 'GCE']:
        ...
        template_path='Migration Pattern_ On-Prem (vSphere) to Google Cloud VMware Engine (GCVE).docx' if destination_environment == 'GCVE' else ('Migration Pattern_ On-Prem (vSphere) to Google Compute Engine (GCE).docx' if destination_environment == 'GCE' else 'Migration Pattern_ On-prem(vSphere) to GKE.docx')
        output_path = '/'.join(json_file_path.split('/')[:-1]) + template_path
        utils.download_gcs_file(artifact_bucket, f'migration-pattern-doc-template/{template_path}', output_path)
        utils.update_document(json_file_path, output_path)
        utils.upload_to_gcs(artifact_bucket, output_path, f'reports/{template_path}')
        return f'https://storage.cloud.google.com/{artifact_bucket}/reports/{template_path}?authuser=1'
...
5. Return Output: Respond only with the GCS URL returned by the update_pattern tool.
